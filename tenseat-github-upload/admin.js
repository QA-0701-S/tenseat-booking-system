...
function applyRestaurant() {
  ...
  el.planStatus.textContent = restaurant.subscriptionStatus === "active" ? "已订阅" : "试用中";
...
function renderBookings(result) {
  ...
  result.bookings.forEach(function (booking) {
    ...
    row.appendChild(createCell(booking.name || "—", "guest-name"));
    row.appendChild(createCell(booking.time, "time-value"));
...
async function jsonRequest(path, options, needsAuth) {
  ...
  if (!response.ok || !result.ok) {
    var error = new Error(result.error || "请求失败");
